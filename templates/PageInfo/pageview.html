...
<!-- Add Group For Page Button (ลิงก์ไปยัง create_group view) -->
<li class="menu-item">
              ...
              <div class="d-flex gap-2 text-muted align-items-center" style="font-size: 0.7rem;">
                {% if post.reactions %}
                  {% for reaction, count in post.reactions.items %}
                    ...
                    {% if reaction == "ขำ" or reaction == "ฮา" %}<span>😂 {{ count }}</span>{% endif %}
                  {% endfor %}
                {% endif %}
...
const scatterOptions = {
  ...
  // ✅ ตรวจสอบไม่ให้ล้นขอบขวาของหน้าจอ
  const screenWidth = window.innerWidth;
  if (left + tooltipWidth > screenWidth) {
    left = screenWidth - tooltipWidth - 20; // -20 เป็น padding ด้านขวา
  }
...
document.addEventListener("DOMContentLoaded", function () {
  // 🎨 Bar Chart: Number of posts by days
